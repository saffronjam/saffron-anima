...
the engine sees a small `sa::` facade of plain structs and `Result`-returning functions, and the
...
| `script-components-and-runtime` | `ScriptComponent` slots, the class-table script shape, the play lifecycle, error containment + the drain commands, the `sa`/entity API reference | `script_runtime.cpp`; `scene_edit_context.cppm`; `host.cppm` |
