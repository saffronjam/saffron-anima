...
| `scene-queries` | raycast + sphere-cast against the live narrow phase (entity-uuid mapping, read-only off the step), the three surfaces (control command, `sa`, the Host-bridged `sa.raycast` Lua binding), and why queries refuse in Edit | `physics.cpp`; `control_commands_physics.cpp`; `script_runtime.cpp` |
