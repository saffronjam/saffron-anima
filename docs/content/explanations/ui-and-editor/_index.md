...
| [`script-logs-panel`](script-logs-panel/) | the play-mode `sa.log` feed (open-AND-playing drain, windowed list), the `AnimaSearchbar` chip search with an `Entity:` typed verb, and the engine `sa.log` → ring → `drain-script-logs` bridge | `ScriptLogsPanel.tsx` · `components/anima/` · `scene_edit_play.cpp` · `control_commands_scene.cpp` |
