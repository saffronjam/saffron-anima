...
// plus a few prelude `rawset(se, "name", ...)`), and documented for VS Code in the `SeLuaDefs` ---@meta
...
// absent from SeLuaDefs. It checks names only (presence), not signatures — that is enough to catch the
...
// se.log is added directly on the global table in newScriptVm.
...
for (const m of runtime.matchAll(/rawset\(se,\s*"([a-z_]+)"/g)) liveNames.add(m[1]);

// SeLuaDefs is a raw-string constexpr; pull its body out of assets.cppm.
const defsMatch = assets.match(/SeLuaDefs\s*=\s*\n?\s*R"\(([\s\S]*?)\)";/);
if (!defsMatch) {
  console.error("check-script-defs: could not locate the SeLuaDefs ---@meta string in assets.cppm");
  process.exit(2);
}
// library/se.lua is SeLuaDefs (hand-written) followed by SeComponentDefs (the generated component-types
...
if (missing.length > 0) {
  console.error("check-script-defs: live Lua bindings missing from library/se.lua (SeLuaDefs):");
  for (const n of missing) console.error("  - " + n);
  console.error("\nAdd them to the SeLuaDefs ---@meta string in assets.cppm.");
  process.exit(1);
}

// The `se.ComponentName` alias is the typed name set for get/set/add/remove/has_component. It must cover
...
const aliasMatch = defs.match(/---@alias se\.ComponentName\s+([^\n]+)/);
...
if (missingComponents.length > 0) {
  console.error("check-script-defs: registered components missing from the se.ComponentName alias:");
  for (const n of missingComponents) console.error("  - " + n);
  console.error("\nAdd them to the ---@alias se.ComponentName line in assets.cppm (SeLuaDefs).");
  process.exit(1);
}
