...
- `docs/hugo.toml`: `baseURL` `…/saffron-engine/` → `…/saffron-anima/`; site `title`
  `saffron-engine` → `saffron-anima`; `BookRepo` `…/saffron-engine` → `…/saffron-anima`.
...
- `docs/content/`: ~74 of 198 pages mention the brand. Rename **product** mentions
  "SaffronEngine" / "Saffron Engine" → "Saffron Anima" in titles, H1s, prose, and the `se` CLI
...
- `README.md`: `<h1>SaffronEngine</h1>` → `Saffron Anima`; badge + repo links `saffron-engine` →
  `saffron-anima`; the binary path / product prose → `SaffronAnima` / "Saffron Anima". Keep the
  `saffron-build` toolbox mention.
- `AGENTS.md`: the title, the architecture paragraph (`Saffron::Engine` → `Saffron::Anima`,
  `SaffronEngine` exe → `SaffronAnima`), binary paths, `SAFFRON_ENGINE_BIN` → `SAFFRON_ANIMA_BIN`,
  the `se` CLI → `sa`. Keep `Saffron.<Area>`, `saffron/` paths, `saffron-build`, `@saffron/protocol`,
...
- `.github/workflows/ci.yml`: the header comments naming "SaffronEngine" → "Saffron Anima". Keep the
...
- `tools/ci/check.sh`, `tools/check-control-schema/package.json`, `tools/check-projects/check.sh`,
  `tools/ci/README.md`: the "SaffronEngine" prose → "Saffron Anima". Binary path + `se`→`sa` socket
...
window reads "Saffron Anima"; and a tree-wide grep for `SaffronEngine`, `Saffron::Engine`, `\bse::`,
`se-types`, ` se\.` (Lua), `tools/se`, `SAFFRON_ENGINE_BIN` returns **only** intentionally-retained
matches (none, for these). Then update the README/plan `Status:` lines to COMPLETED.

## External follow-up (user action, not in-tree)

- Rename the GitHub repo `saffron-engine` → `saffron-anima` and update `git remote set-url` (a git
