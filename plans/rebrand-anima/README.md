# Rebrand: Saffron Anima → Saffron Anima

**Status:** COMPLETED

Rename the product from **Saffron Anima** to **Saffron Anima**. "Saffron" is the umbrella brand
...
| Product name | **Saffron Anima → Saffron Anima** |
| Short namespace | **`se` → `sa`** (C++ `sa::`, Lua `sa.`) |
| C++ module names + source dir | **kept** (`Saffron.<Area>`, `engine/source/saffron/`) |
| `SAFFRON_*` env prefix | **kept** (only `SAFFRON_ANIMA_BIN` → `SAFFRON_ANIMA_BIN`) |
| `saffron-build` toolbox + asset extensions | **kept** |
| GitHub repo + docs URL | **renamed** (`saffron-anima` → `saffron-anima`) |
...
| Concept | Old | New |
|---|---|---|
| Product name (prose) | `Saffron Anima` / `SaffronAnima` | `Saffron Anima` / `SaffronAnima` |
| C++ namespace | `sa::` | `sa::` |
| Lua global table | `se.` | `sa.` |
| Host exe target + binary | `SaffronAnima` → `build/debug/bin/SaffronAnima` | `SaffronAnima` → `build/debug/bin/SaffronAnima` |
| Static lib target | `SaffronAnimaLib` | `SaffronAnimaLib` |
| Lib alias | `Saffron::Anima` | `Saffron::Anima` |
| CMake `project()` | `SaffronAnima` | `SaffronAnima` |
| Window title | `"Saffron Anima"` | `"Saffron Anima"` |
| Trace/process name (pid) | `"SaffronAnima"` | `"SaffronAnima"` |
| Engine-binary env var | `SAFFRON_ANIMA_BIN` | `SAFFRON_ANIMA_BIN` |
| Generated TS protocol file | `editor/src/protocol/sa-types.ts` | `editor/src/protocol/sa-types.ts` |
| Lua type defs | `---@class sa.*`, `SaLuaDefs`, `library/sa.lua` | `---@class sa.*`, `SaLuaDefs`, `library/sa.lua` |
| Control CLI | `se` (`tools/sa/`) | `sa` (`tools/sa/`) |
...
| GitHub repo / Hugo `baseURL` | `saffron-anima` | `saffron-anima` |
...
- Env prefix `SAFFRON_*` for all vars **except** `SAFFRON_ANIMA_BIN` (e.g. `SAFFRON_CONTROL_SOCK`,
...
The investigation found the namespace is always accessed as `sa::` (26 `namespace sa` decls, ~43
`sa::` uses; **no** `using namespace sa`, no nested `sa::detail`). So the safe, targeted patterns are
`namespace sa` → `namespace sa` and `sa::` → `sa::`. The Lua side is `se.` (table access) and `"se: "`
...
| 5 | `phase-5-control-cli.md` | `tools/sa/` → `tools/sa/`: binary `se` → `sa`, help text, sockets, references |
...
- **Whole rebrand done when:** `make check` and `make e2e` are green; the editor builds
  ...
  grep for the renamed tokens (`SaffronAnima`, `Saffron::Anima`, `sa::`, ` se\.` in Lua/defs,
  `sa-types`, `SAFFRON_ANIMA_BIN`) returns only intentional, retained matches.
...
- **Generator vs. hand edits.** Generated files (`*.generated.{cpp,hpp}`, `sa-types.ts`,
...
- Rename the GitHub repository `saffron-anima` → `saffron-anima` and update the local `git remote`
