...
`library/sa.lua` + `.luarc.json` scaffold + the gating drift tripwire). **7b (the declarative
...
for a single-source benefit the gating tripwire already delivers. The hand-written `sa.lua` + tripwire
...
read input edges and the mouse, drive animation clips, run a follow camera, schedule work with `sa.wait`,
...
3. **Degrade-to-no-op handles.** `sa.Entity` is a value handle `{ entt id, ScriptHost* }`; it reaches the
...
4. **No legacy.** When a shape is superseded (the `{x,y,z}` tables → `sa.Vec3`), it is **replaced**
...
| `sa.Vec3` userdata replacing all `{x,y,z}` tables | `sa.Quat` userdata — rotation is Euler radians in `TransformComponent.rotation`; a `look_at` helper covers the camera case. Add Quat only when gimbal-free composition is needed. |
...
| `sa.broadcast` / `entity:send` bus + `sa.wait`/scheduler | Roblox `GetPropertyChangedSignal` — no registry change-hook; scripts poll or fire explicit messages. |
| Hand-written `sa.lua` + runtime tripwire | The declarative binding table + generator (Phase 7b) — lifts once the surface stabilizes. |
...
Keep `sa.Entity` exactly as it is: a trivially-copyable value handle, scene reached only via
...
### Vec3 — LOCKED: a real `sa.Vec3` userdata, replacing tables wholesale

**Decision: introduce an `sa.Vec3` userdata backed by `glm::vec3`, and cut every `{x,y,z}` table over to it
...
Plus free helpers `sa.vec3(x,y,z)` (factory alias), `sa.lerp(a,b,t)`, `sa.look_at(eye, target, up?) ->
...
- Setters take **one `Vec3`**: `set_position(v)`, `set_rotation(v)`, `set_scale(v)`. No-legacy: the
  ...
  `set_position(sa.vec3(...))` / `set_position(p + …)` in the same change, and the scaffold orbit math +
  its `Math.hypot` assertion (`script.test.ts:416`) to rebuild `self.center` from `Vec3`, not `{x=…}`.
- A `vec3` **declared field** default is written `offset = sa.vec3(0, 1, 0)`. So **`sa.Vec3` must be
  registered in `newScriptVm` too** (`script.cppm:230`–`246`, today only `sa.log`), not only `startScripts`
...
| `sa.look_at(eye, target, up?)` | glm + `quatToEulerZYX` | **pure binding** (math) |
...
| API | Backing | Tag |
|---|---|---|
| `sa.spawn(name) -> sa.Entity` | `createEntity` (`scene.cppm:618`) | **pure binding** |
...
| `entity:parent() -> sa.Entity\|nil` / `entity:children() -> {sa.Entity}` | `RelationshipComponent` caches | **pure binding** (read walk) |
| `sa.get_entity_by_name(name) -> sa.Entity\|nil` | `forEach<NameComponent>` (the existing binding, `:451`) | **pure binding** (unchanged name) |
| `sa.find_all_by_name(name) -> {sa.Entity}` | `forEach<NameComponent>` (multi-match) | **pure binding** (new, distinct purpose) |
| `sa.find_by_uuid(uuid) -> sa.Entity\|nil` | `findEntityByUuid` (`:735`) | **pure binding** |

**Name discipline (no-legacy).** The existing first-match lookup stays named `sa.get_entity_by_name`
(`script_runtime.cpp:451`); the plan does **not** introduce a second `sa.find_by_name` for the same
`NameComponent` lookup. The genuinely new surface is the **multi-match** `sa.find_all_by_name` (a distinct
...
| Lua API | Backing | Tag |
|---|---|---|
| `sa.raycast(...)` | `raycastWorld` (`physics.cppm:135`) | exists |
| `sa.spherecast(origin, dir, radius, maxDist) -> RayHit` | `sphereCastWorld` (`physics.cppm:139`) | **bridge** — `ScriptHost::sphereCast`, reuses `ScriptRayHit` POD verbatim |
...
`sa.is_key_pressed(key)` reads `ScriptHost::inputKeys` (`script.cppm:104`), a held-key set **forwarded from
...
|---|---|---|
| `sa.is_key_pressed(key)` (held) | — | exists |
| `sa.just_pressed(key)` / `sa.just_released(key)` (edge) | host diffs this-tick vs last-tick held sets into two derived sets pushed alongside `inputKeys`; the editor already sends held keys | **NEW C++** (host plumbing — small) |
| `sa.mouse_position() -> Vec3` / `sa.mouse_delta() -> Vec3` / `sa.mouse_button(n) -> bool` / `sa.mouse_scroll() -> number` | the `script-input` DTO (`control_dto.cppm`, today only `keys`) + `ScriptHost` input state widened to carry mouse; the editor forwards it (the engine window has no mouse signal, `window.cppm:29`–`33`) | **NEW C++** (DTO + editor + host) |
...
`target:get_world_position() + offset`, orients with `sa.look_at`, and uses `sa.raycast` for occlusion
...
- `sa.wait(seconds)` — yields the current coroutine; the runtime resumes it once accumulated `dt` passes the
  deadline. Logged error if called outside a coroutine.
- `sa.spawn_task(fn, ...)` — start a managed coroutine, resumed immediately (Roblox `task.spawn`).
- `sa.delay(seconds, fn)` — the callback form (Defold `timer.delay` / Roblox `task.delay`).
...
## 6. The `sa.lua` def file + `.luarc.json` + scaffold

### The fork — LOCKED: hand-write `sa.lua` + a runtime tripwire NOW; declarative table + generator in Phase 7b
...
call arguments and lambda bodies (and `sa.log` is bound in a **second** TU, `newScriptVm`, `script.cppm:238`).
A pure source-parse would be brittle and silently wrong. So:

1. **Phases 1–6 hand-write `library/sa.lua`** (`---@meta`, types only) **plus a runtime-introspection
   ...
   `se` global table **and** the `sa.Entity` metatable for every exposed name (covering **both**
   registration TUs — `newScriptVm`'s `sa.log`/`sa.Vec3` and `startScripts`'s Entity/raycast block), parse
   `sa.lua`'s `---@field`/`---@class`/method names, and **fail if the live surface has a name `sa.lua`
...
2. **Phase 7b lifts** the binding list into **one declarative C++ table** that both the LuaBridge
   registration loop **and** an `sa.lua` generator consume (the `gen.ts` one-source-of-truth model). The
   tripwire then becomes a "generated `sa.lua` is byte-fresh" `git diff` check identical to the DTO gate,
   and the hand-written `sa.lua` is **deleted** (no-legacy — the generated file replaces it). Deferred to
...
`phase-7-luals-defs-and-codegen.md`; in summary it covers `sa.Vec3` (operators via `---@operator`),
`sa.RayHit`, `sa.Entity` (every method from Phases 1–6), `sa.ScriptSelf` (the `self` shape), and the `se`
...
- New `ensureScriptLibrary(root)` modeled on `ensureScriptSrc` (`assets.cppm:1057`): `create_directories`
  `root/"library"`, write `library/sa.lua` from a new `inline constexpr std::string_view SaLuaDefs`
...
- `StarterScript` (`:1029`) and the `createProjectScript` template (`:1103`) switch to **colon methods** +
  `---@class Example : sa.ScriptSelf` so `self.entity:get_position()` autocompletes. This is **runtime-
  ...
  body also moves to `sa.Vec3` orbit math (Phase 2) in the same cutover.
...
`tests/e2e/script.test.ts` cases (clean-log assertion stays green), a scaffold/`sa.lua` touch, and a
...
| 2 | `phase-2-vec3-userdata.md` | `sa.Vec3` userdata (read-write props, dual `__mul`, math), registered in **both** `newScriptVm` and `startScripts`, cut through every vector API + `inferField`/`injectFields` + scaffold + e2e | **pure binding** | 1 (uses world getters) |
...
| 6 | `phase-6-events-timers.md` | `entity:send`/`sa.broadcast` bus + `sa.wait`/scheduler (coroutine lib), resumed **inside** the tick window | host-struct state (no new module) | 3 (flush discipline) |
| 7 | `phase-7-luals-defs-and-codegen.md` | **7a** hand-written `library/sa.lua` + `.luarc.json` scaffold + colon-method/`---@class` switch + **gating tripwire** in `check.sh`; **7b** declarative binding table + generator, delete hand-written `sa.lua` | tooling | 1–6 (surface stable) |

**Note on 7a sequencing:** the `sa.lua` + `.luarc.json` + tripwire can land **early** (right after Phase 1)
...
1. **Vec3 cutover completeness.** `sa.Vec3` must be bound in **both** `newScriptVm` (so
   `Class.properties = { x = sa.vec3(...) }` works in the `readScriptSchema` throwaway VM) **and**
...
7. **Input source-of-truth singularity.** Edges + mouse come over the **one** `script-input` control-plane
   channel (not a second SDL feed); headless e2e must still drive them. Confirm no `sa.gamepad_*` is bound
   or annotated (no engine path).
8. **No duplicate name.** `sa.get_entity_by_name` stays the single first-match name (no second
   `sa.find_by_name`); only `sa.find_all_by_name`/`sa.find_by_uuid` are added as distinct-purpose bindings.
9. **`sa.lua` tripwire is gating and covers both TUs.** It must run as a **gating** step in `check.sh`
   (not optional) and enumerate `sa.log`/`sa.Vec3` (bound in `newScriptVm`, `script.cppm:238`) alongside
...
11. **Scaffold idempotence.** `library/sa.lua` + `.luarc.json` written only-when-absent at both
