...
## New `se` globals + `se.Entity` methods (LOCKED)

| Surface | Signature | Backing (`scene.cppm`) | Notes |
|---|---|---|---|
| `se.spawn(name)` | `-> se.Entity` | `createEntity` (`:618`) | mints a fresh uuid; entity lives in the play duplicate only |
...
| `entity:parent()` | `-> se.Entity \| nil` | `RelationshipComponent.parent` + `findEntityByUuid` (`:735`) | nil at root |
| `entity:children()` | `-> { se.Entity }` | `RelationshipComponent.children` cache (relinkHierarchy-fresh) | |
| `se.find_all_by_name(name)` | `-> { se.Entity }` | `forEach<NameComponent>` (the existing `:451` walk, multi-match) | the multi-match the existing `get_entity_by_name` can't give |
| `se.find_by_uuid(uuid)` | `-> se.Entity \| nil` | `findEntityByUuid` (`:735`) | uuid lookup |
...
**Name discipline (no-legacy, LOCKED).** The existing first-match `se.get_entity_by_name`
(`script_runtime.cpp:451`) **stays as-is** — the plan does **not** add a duplicate `se.find_by_name` for the
same `NameComponent` lookup. The new surface is the **multi-match** `se.find_all_by_name` (a distinct purpose
...
matrix; ship `find_all_by_name` now. Do **not** annotate or bind any `find_by_tag` in `se.lua`.
...
- `se.spawn` adds to the duplicate; the entity **vanishes on stop** by design. Correct (the authored scene
...
- `se.spawn` / `entity:destroy` / `entity:set_parent` **enqueue** an op and set `hierarchyDirty`; they do
  **not** touch `entt` storage during the loop. `se.spawn` returns the freshly-minted handle immediately (it
...
2's world getters + `se.look_at` + `se.lerp` and this phase's `set_parent`/spawn. Ship a `follow_camera.lua`
recipe in the camera/scripting docs page (and optionally a scaffold example): a separate camera entity whose
script lerps its world position toward `target:get_world_position() + offset`, orients with `se.look_at`, and
uses `se.raycast` for occlusion pull-in.

## Control command (drivable-state rule)

`se.spawn` / `destroy` / `set_parent` map to existing control commands (`add-entity`, `destroy-entity`,
...
- A spawner script `se.spawn("Bullet")`, adds a Transform, sets a position; during play the entity is present
