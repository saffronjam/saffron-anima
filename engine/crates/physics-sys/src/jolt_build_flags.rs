...
//     `JPH_USE_*` defines, emitted in lockstep. The defines and the `-m` flags MUST stay
//     paired: a define without its flag (or vice versa) is a silent ABI skew.
//   - `-ffp-model=precise -ffp-contract=off` is the determinism FP pairing.
