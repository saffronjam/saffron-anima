...
//!
//! This ports `uploadMesh` (`renderer_drawlist.cpp:76`/`:81`), `uploadTexture` and
//! `uploadTextureFloat` (`renderer_textures.cpp:145`/`:302`), plus the one-off
//! command submit they share (`submitAndWait` / `withOneOffCommands`,
//! `renderer_detail.cppm:1203`/`:1231`). The C++ ran these as free functions on the
//! `Renderer&`; here they hang off an [`Uploader`] that owns the one-off command
//! pool and a clone of the externally-synchronized [`GpuQueue`] — the README §5
//! first shared-mutable site, so the worker thread can upload off the main thread.
//!
...
///
/// README §5's first `Arc<Mutex>` site (the C++ `gpuQueueMutex`,
/// `renderer_types.cppm:33`): the frame loop's submit/present and the worker
...
impl GpuQueue {
    ...
    /// held only for the submit call (the C++ `gpuQueueMutex` scope).
    ///
    /// # Errors
    ///
...
/// matching the C++ `withOneOffCommands`. [`Drop`] frees the pool.
pub struct Uploader {
    resources: Arc<DeviceResources>,
    queue: GpuQueue,
    command_pool: vk::CommandPool,
    /// The acceleration-structure dispatch for building a per-mesh BLAS at upload time when
    /// RT is supported (the C++ `buildBlas` in `uploadMesh`). `None` on a software device —
...
impl Uploader {
    ...
    ///
    /// The C++ `withOneOffCommands` (`renderer_detail.cppm:1231`).
    ...
    /// scratch is held across the submit then dropped. `None` on a software device. The
    /// C++ `buildBlas` invoked from `uploadMesh`.
    fn build_mesh_blas(
        &self,
    ...
    /// compute skinning prepass reads it). The C++ `uploadMesh`.
    ///
    /// # Errors
    ///
    ...
    pub fn upload_mesh(&self, mesh: &Mesh, skin: &[VertexSkin]) -> Result<Arc<GpuMesh>> {
        ...
        // device address + AS-build-input usage (the C++ `rtUsage`).
    ...
    /// chain is generated by blitting down from mip 0. The C++ `uploadTexture`.
    ///
    /// # Errors
    ///
    ...
    /// returning the [`GpuTexture`] the renderer holds for its lifetime (the C++
    ...
    /// unbound. The C++ default-white upload + fill in `initRenderer` (`renderer.cppm:532`).
    ///
    /// # Errors
    ///
...
impl<'a> StagingBuffer<'a> {
    ...
    fn flush(&self) {
        // The map may be coherent; the flush is a no-op then. Either way it is the
        // C++ `vmaFlushAllocation(0, WHOLE_SIZE)`.
        let _ = self
            .allocator
...
/// read. The C++ `uploadTexture` body + `recordMipChain`.
///
/// # Safety
///
...
///
/// A bit-exact port of the C++ `floatToHalf` (`renderer_textures.cpp:265`).
...
mod tests {
    ...
    /// The mip-chain length matches the C++ `mipCount`: 1×1 → 1, square powers of two
    /// → log2+1, and a non-square image uses the larger dimension.
    #[test]
    ...
    /// `float_to_half` reproduces the known IEEE half encodings the C++ produced: the
    /// exact representables, the f16-max overflow to +inf, and the sign bit. This is
